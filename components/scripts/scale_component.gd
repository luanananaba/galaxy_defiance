...
func tween_scale() -> void:
	...
	# Next we scale the sprite from its current scale to the scale amount
	# (in 1/10th of the scale duration).
	...
	# Consider that we always scale back to a value of 1, you could store the starting scale
	# amount for the sprite as well for games where your character doesn't start with a scale of 1.
